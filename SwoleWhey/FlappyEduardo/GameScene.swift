...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func createChaChingSound() -> SystemSoundID {
        ...
        let soundURL = CFBundleCopyResourceURL(CFBundleGetMainBundle(), "Eddy", "mp3", nil)
    ...
    func didBeginContact(contact: SKPhysicsContact) {
        ...
        if firstBody.categoryBitMask == PhysicsCatagory.Score && secondBody.categoryBitMask == PhysicsCatagory.Ghost{
            ...
            score++
            scoreLbl.text = "\(score)"
            runAction(SKAction.playSoundFileNamed("Eddy2.mp3", waitForCompletion: false))
            ...
            enumerateChildNodesWithName("wallPair", usingBlock: ({
                (node, error) in
                //runAction(SKAction.playSoundFileNamed("Eddy.mp3", waitForCompletion: false))
                node.speed = 0
                self.removeAllActions()
